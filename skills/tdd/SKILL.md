...
description: >-
  ...
  Triggers: tdd, test driven development, test first, red green refactor, write
  tests first, what makes a good test, test seam, where to test, tautological test,
  test implementation vs behavior, vertical slice test, tracer bullet, testing
  discipline, why do my tests break on refactor.
  Not for: autonomous test generation and healing, full-repo test lifecycle and CI
  gates (see the testing skill), browser E2E, or verifying code against a product
  spec.
---
...
reaching inside. Tests live at seams, never against internals. (For the full vocabulary around
seams and module depth, see software-design literature on deep modules — e.g. Ousterhout / Feathers.)
...
- **Tautological** — the assertion recomputes the expected value the way the code does
  ...
  values must come from an independent source of truth — a known-good literal, a worked example,
  the spec.
...
This is **discipline**, not tooling. Nearby tools:

| Need | Skill |
|------|-------|
| Test strategy and pre-deploy quality gates | `testing` |
| Runner-specific APIs (Vitest, PHPUnit) | `vitest`, `phpunit` |

Sources: https://github.com/mattpocock/skills (engineering/tdd, MIT)
